...
import { UserPlus, Loader2, ArrowRight, Mail, Phone, ShieldCheck, RotateCcw } from "lucide-react";
...
const RegisterPage = () => {
    ...
    const { register, loginWithPhone, loading, error, clearError } = useAuth();
            ...
            try {
                if (!verifierRef.current && recaptchaContainerRef.current) {
                    ...
                    v.render().then(() => {
                        ...
                        window.recaptchaVerifier = v; // Keep for back-compat
                    });
                }
            } catch (err) {
                    ...
                    {tab === "email" && (
                        <motion.div key="email-tab" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }}>
                            <div className="text-center mb-8">
                                ...
                                <p className="text-gray-500 mt-1 text-sm">Register with your email to get started</p>
                            </div>

                            {displayError && (
                                <div className="mb-5 p-3.5 bg-red-50 border border-red-100 rounded-xl text-sm text-red-600 font-medium">{displayError}</div>
                            )}

                            <form onSubmit={handleEmailRegister} className="space-y-5">
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Full Name</label>
                                    <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="John Doe" disabled={loading}
                            ...
                            <p className="text-center text-sm text-gray-500 mt-6">
                                ...
                                <Link href="/login" className="text-blue-600 font-semibold hover:text-blue-700">Login In</Link>
                            </p>
                        </motion.div>
                    )}
                                ...
                                {phoneStep === "input" && (
                                    <motion.div key="phone-input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                        <div className="text-center mb-8">
                                            ...
                                            <p className="text-gray-500 mt-1 text-sm">Enter your mobile number to get started</p>
                                        </div>
                                ...
                                {phoneStep === "otp" && (
                                    <motion.div key="otp-verify" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                        ...
                                        <button onClick={verifyOTP} disabled={loading || otp.join("").length !== 6}
                                            ...
                                            {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Verifying...</> : <>Verify & Register <ShieldCheck className="w-5 h-5" /></>}
                                        </button>
